...
impl PolyComm {
    ...
    pub fn setup(d_max: &BigInt) -> PP {
        // TODO: use standardize setup for sampling random random element
    ...
    pub fn commit(pp: &PP, coef_vec: &[FE]) -> (PolyComm, BigInt){
        ...
        let f_q = tail.fold(head.clone(), |acc, x| {
            x +  &acc * &pp.q
        });
    ...
    pub fn decode(p: &BigInt, q: &BigInt, y: &BigInt) -> Vec<FE> {
        ...
        let mut f_0: BigInt = (s_0 - s_minus_1);
        //f_0 = f_0.mod_floor(p);
    ...
    pub fn eval_bounded_prove(c: &BinaryQF, pp: &PP, z: &FE, y: &FE, d: usize, b: BigInt, coef_vec: &mut [BigInt]) -> NiEvalProof{
        ...
        let y_l = tail.fold(head.clone(), |acc, x| {
               x +  &acc * &z_bn
        });
        ...
        let y_r = tail.fold(head.clone(), |acc, x| {
            x +  &acc * &z_bn
        });
        ...
        let head = f_l_coef_rev.next().unwrap();
        let tail = f_l_coef_rev;
        let f_l_q = tail.fold(head.clone(), |acc, x| {
            x +  &acc * &pp.q
        });
        ...
        let head = f_r_coef_rev.next().unwrap();
        let tail = f_r_coef_rev;
        let f_r_q = tail.fold(head.clone(), |acc, x| {
            x +  &acc * &pp.q
        });
        ...
        //step 19
           ...
           poe_proof_vec.push(poe_proof);
            ...
            //step 22
            let eval_proof = Self::eval_bounded_prove(&c_prime, &pp, z, &y_prime_fe, d_prime, b_prime, & mut coef_vec_int_prime[..] );
            y_l_vec.extend_from_slice(&eval_proof.y_l_vec[..]);
            y_r_vec.extend_from_slice(&eval_proof.y_r_vec[..]);
            c_l_vec.extend_from_slice(&eval_proof.c_l_vec[..]);
            c_r_vec.extend_from_slice(&eval_proof.c_r_vec[..]);
...
impl NiEvalProof{
    ...
    pub fn eval_verify(mut self, c: BinaryQF, pp: &PP, z: &FE, y: &FE,) ->  Result<(),(ErrorReason)>{
        ...
        if  (self.d.clone() + 1) % 2  == 1 {
            ...
            match result{
                Ok(()) => return Ok(()),
                Err(x) => return Err(ErrorReason::EvalError),
